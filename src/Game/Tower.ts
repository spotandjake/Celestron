...
export default class Tower {
  ...
  public Update(deltaTime: number, world: World) {
    const enemiesInRange = new Map(
      [...world.enemys.entries()].filter(([key, enemy]) => {
    ...
    const targetAngle = targetEnemy.Position.clone().subVector(this.Position).normalize();
    const direction = Math.atan2(targetAngle.y, targetAngle.x);
    this.Direction +=
      ((direction - this.Direction) / (this.speed * (world.worldSpeedScalar * 2))) * 2;
    ...
    if (this.TowerType == TowerType.Machine && this.currentFireCycle < 0) {
      for (let i = 10; i--; ) {
        world.particles.set(
          ...
          new Particle(
            world,
            ParticleType.Smoke,
            this.Position.x + Math.cos(this.Direction) * 50,
            this.Position.y + Math.sin(this.Direction) * 50,
      ...
      world.particles.set(
        world.particles.size,
        new Particle(world, ParticleType.MgunBullet, this.Position.x, this.Position.y, {
          rotation: this.Direction,
        }),
      );
    } else if (
      this.Direction - direction > -Math.PI / 16 &&
      this.Direction - direction < Math.PI / 16 &&
      this.currentFireCycle < 0
    ) {
      targetEnemy.health -= this.damage;
      if (targetEnemy.health <= 0) this.kills++;
      this.currentFireCycle = this.fireRate;
      if (this.TowerType == TowerType.Basic) {
        ...
        world.particles.set(
          world.particles.size,
          new Particle(world, ParticleType.Bullet, this.Position.x, this.Position.y, {
            rotation: this.Direction,
          }),
        );
      } else {
        ...
        world.particles.set(
          world.particles.size,
          new Particle(world, ParticleType.RailgunBullet, this.Position.x, this.Position.y, {
            rotation: this.Direction,
          }),
        );
      }
