    ...
    if (this.currentPath.points.length > 0) {
      ...
      if (
        !movement
          .normalize()
          .round()
          .equals(this.Position.clone().subVector(target).normalize().round())
      )
        this.Position.copy(target);
      // Check if we reached target
      if (
        this.Position.distanceToSquared(this.currentPath.points[0]) <
        (deltaTime / 50) * this.speed
      )
      ...
      if (this.currentPath.exitSegment) {
        this.currentPath = world.getPath(
          this.currentPath.exitSegment,
          this.Position,
